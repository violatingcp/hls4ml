...
void myproject_in(
  ...
  #pragma HLS DATAFLOW
    ...
    while(!layer26_out[0].empty()) nnet::conv_2d_large_cl2<layer26_t, layer30_t, config27>(layer26_out, layer30_out, w27, b27);

    static hls::stream<layer34_t> layer34_out[N_FILT_31];
    #pragma HLS STREAM variable=layer34_out depth=50 dim=1
    while(!layer30_out[0].empty()) nnet::conv_2d_large_cl2<layer30_t, layer34_t, config31>(layer30_out, layer34_out, w31, b31);
    ...
    while(!layer35_out[0].empty()) nnet::conv_2d_large_cl2<layer35_t, layer39_t, config36>(layer35_out, layer39_out, w36, b36);

    static hls::stream<layer43_t> layer43_out[N_FILT_40];
    #pragma HLS STREAM variable=layer43_out depth=50 dim=1
    while(!layer39_out[0].empty()) nnet::conv_2d_large_cl2<layer39_t, layer43_t, config40>(layer39_out, layer43_out, w40, b40);
