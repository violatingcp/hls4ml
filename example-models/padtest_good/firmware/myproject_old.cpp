...
void myproject_in(
  ...
  const_size_out_1 = 1;//OUT_HEIGHT_6*OUT_WIDTH_6*N_FILT_6;
    ...
    while(!layer3_out[0].empty()) nnet::relu_stream<layer3_t, layer4_t, Relu_config4>(layer3_out, layer4_out);


    static hls::stream<layer5_t> layer5_out[N_FILT_5];
    #pragma HLS STREAM variable=layer5_out depth=1 dim=1
    while(!layer4_out[0].empty()) nnet::pad_2d_cl<layer4_t, layer5_t, config5>(layer4_out, layer5_out);
