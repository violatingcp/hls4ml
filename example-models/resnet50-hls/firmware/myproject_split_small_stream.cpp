...
#include "myproject_split_small_stream.h"
...
void subimage(bool iReset,
  ...
  hls::stream<result_t> output[N_LAYER_OUT_3]) { 
    ...
    // ****************************************
    static layer2_t layer2_in_row[N_SUB_INPUT_2_1][N_FILT_HEIGHT_2][N_INPUT_1_1];
    #pragma HLS ARRAY_RESHAPE variable=layer2_in_row complete dim=2

    static layer2_t layer2_in[N_FILT_HEIGHT_2*N_FILT_WIDTH_2*N_INPUT_1_1];
    #pragma HLS ARRAY_RESHAPE variable=layer2_in complete dim=0

    static layer2_t layer2_out[N_FILT_2];
    #pragma HLS ARRAY_RESHAPE variable=layer2_out complete dim=0

    static layer3_t layer3_out[N_FILT_2];
    #pragma HLS ARRAY_RESHAPE variable=layer3_out complete dim=0

    static layer4_t layer4_out[N_FILT_2];
    #pragma HLS ARRAY_RESHAPE variable=layer4_out complete dim=0

    static layer5_t layer5_in_row[N_SUB_OUT_WIDTH_2][N_FILT_HEIGHT_5][N_FILT_2];
    #pragma HLS ARRAY_RESHAPE variable=layer5_in_row complete dim=2

    static layer5_t layer5_in[N_FILT_HEIGHT_5*N_FILT_WIDTH_5*N_FILT_2];
    #pragma HLS ARRAY_RESHAPE variable=layer5_in complete dim=0

    static layer5_t layer5_out[N_FILT_5];
    ...
    //Processs image
    if(iY > N_SUB_INPUT_3_1_C-2 && iX == 0) { 
     LoopX:
     for(unsigned i1 = 0; i1 < N_SUB_INPUT_2_1_C; i1++) { 
      ...
      LoopY:
      for(unsigned i1 = 0; i1 < config2::pad_left+1; i1++) { 
       #pragma HLS PIPELINE II=2
       for(unsigned i2 = 0; i2 < N_INPUT_1_1;   i2++) { 
        layer2_in_row[i1+config2::pad_left]  [N_FILT_HEIGHT_2-2][i2]     =  input[i2].read();
        layer2_in_row[i1+config2::pad_left]  [N_FILT_HEIGHT_2-1][i2]     =  input[i2].read();
       }
       //Clear convs
     ...
     } else if(iX < N_SUB_INPUT_2_1_C-config2::pad_right) { //or we feed it zeros
      LoopZ:
       for(unsigned i2 = 0; i2 < N_INPUT_1_1;   i2++) { 
        #pragma HLS UNROLL
        layer2_in_row[iX+config2::pad_left]  [N_FILT_HEIGHT_2-2][i2]     =  input[i2].read();
        layer2_in_row[iX+config2::pad_left]  [N_FILT_HEIGHT_2-1][i2]     =  input[i2].read();
       }
     }
     ...
     nnet::fill_entry_2d<layer4_t,layer5_t,config5>(iX,layer4_out,layer5_in_row);
     nnet::shift_right_stride_2d<layer2_t,layer2_t,config2>(iX,layer2_in_row,layer2_in);
     nnet::maxpool2d_filt_cl<layer5_t, config5>(layer5_in, layer5_out);
     if(iX > config5::pad_left && iX % 2 == 1) { 
       nnet::fill_image_2dS1<layer5_t,result_t,config5>(layer5_out,output);
     } 
     if(iX % 2 == 0 && iX > config2::pad_left) pCX = pCX+1;
     nnet::shift_right_stride_2d<layer5_t,layer5_t,config5>(iX,layer5_in_row,layer5_in);
     if(iX % 2 == 0) pCY = pCY+1;
     iX = iX+1;
     if(iX == N_SUB_INPUT_2_1_C) { 
       iX = 0;
       iY = iY+1;
       pCX = 0; 
       //Shiftdowns
