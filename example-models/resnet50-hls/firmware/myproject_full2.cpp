...
void myproject(
  ...
  */
    ...
    layer2_t layer2_in_row[N_FILT_2*OUT_WIDTH_2*N_FILT_HEIGHT_2];
    #pragma HLS ARRAY_RESHAPE variable=layer2_out_row complete dim=0
    for(unsigned i0 = 0; i0 < N_FILT_2*OUT_WIDTH_2*N_FILT_HEIGHT_2; i0++) { 
      layer2_in_row[i0] = 0;
    }

    layer6_t layer2_in[N_FILT_HEIGHT_2*N_FILT_WIDTH_2*N_INPUT_1_1];
    ...
    #pragma HLS ARRAY_PARTITION variable=layer5_in complete dim=0

    layer6_t layer6_out[N_FILT_6];
    #pragma HLS ARRAY_PARTITION variable=layer6_out complete dim=0

    layer6_t layer6_in_row[N_FILT_6*N_FILT_HEIGHT_6*OUT_WIDTH_6];
    #pragma HLS ARRAY_PARTITION variable=layer6_out_row complete dim=0
    ...
    #pragma HLS ARRAY_RESHAPE variable=layer9_out_row complete dim=0
    ...
    for(unsigned i0 = 0; i0 < N_INPUT_2_1_C; i0++) { 
	 ...
	 //std::cout << " --> " << i0 << " -- " << i1 << " -- " << i2 << " -- " << (i1*config2::n_chan*config2::filt_height+i2*config2::filt_height) << " -- " << (N_FILT_2*OUT_WIDTH_2*N_FILT_HEIGHT_2) << " -- " << (i0*N_INPUT_2_1*N_INPUT_3_1+i1*N_INPUT_3_1+i2) << std::endl;
	 //std::cout << " -2--> " << layer2_in_row[i1*config2::n_chan*config2::filt_height+i2*config2::filt_height] << " -- " << (N_FILT_2*OUT_WIDTH_2*N_FILT_HEIGHT_2) << std::endl;
	 //std::cout << " -3--> " <<  gpu_0_data_0[i0*N_INPUT_3_1*N_INPUT_1_1+i1*N_INPUT_1_1+i2] << " -- " << (N_INPUT_1_1*N_INPUT_2_1*N_INPUT_3_1)  << std::endl;
	...
	layer176_out[i0*56+i1] =  gpu_0_data_0[i0*N_INPUT_3_1*N_INPUT_1_1+i1*N_INPUT_1_1];
      }
    }
      /*
      //Clear convs
    ...
    for(int i0=0; i0 < OUT_HEIGHT_174*OUT_WIDTH_174*N_FILT_174; i0++) { 
      #pragma HLS PIPELINE
      layer174_out[i0] = gpu_0_data_0[i0];
    }
