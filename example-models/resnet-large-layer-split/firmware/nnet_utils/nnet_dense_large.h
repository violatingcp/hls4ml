...
template<class data_T, class res_T, typename CONFIG_T>
void dense_large_rf_gt_nin_rem0_nobias(
    data_T data[CONFIG_T::n_in],
    res_T  res[CONFIG_T::n_out],
    typename CONFIG_T::weight_t weights[CONFIG_T::n_in*CONFIG_T::n_out]) {
    ...
    //int in_index = 0;
    //int out_index;
    //unsigned short outstep = 0;
    //const int outscale = rufactor / nin;
    //unsigned short outidx[rufactor];
    //IndexLoop:
    //for (int ir = 0; ir < rufactor; ir++) {
    //    outidx[ir] = outstep;
    //    if ((ir + 1) % nin == 0) {
	...
	typename CONFIG_T::accum_t tmpmult[block_factor];
        ...
        for (int im = 0; im < block_factor; im++) {
            #pragma HLS UNROLL
            tmpmult[im] = product<data_T, typename CONFIG_T::weight_t, typename CONFIG_T::accum_t>(data[ir], weights[w_index]);
            w_index += rufactor;
...
template<class data_T, class res_T, typename CONFIG_T>
void dense_large_rf_gt_nin_rem0_nobias_str(
      data_T data[CONFIG_T::n_in],
      res_T  res[CONFIG_T::n_out],
      hls::stream<typename CONFIG_T::weight_t> weights[CONFIG_T::n_out]) {

  //    #pragma HLS resource core=AXI4Stream variable=weights
	...
	typename CONFIG_T::accum_t tmpmult[block_factor];
        ...
        for (int im = 0; im < block_factor; im++) {
            #pragma HLS UNROLL
   	    tmpmult[im] = product<data_T, typename CONFIG_T::weight_t, typename CONFIG_T::accum_t>(data[ir], weights[im].read());
            //w_index += rufactor;
            //if (w_index >= CONFIG_T::n_in * CONFIG_T::n_out) break; // check out of bounds
        }
        for (int im = 0; im < block_factor; im++) {
...
template<class data_T, class res_T, typename CONFIG_T>
void dense_large_nobias_4608sel(
    data_T data[4608],
    ...
    for (unsigned int i = 0; i < 36; i++) {
        ...
        data_sel[i] = data[(index*36)+i];
    }

    dense_large_rf_gt_nin_rem0_nobias<data_T, res_T, CONFIG_T>(data_sel, res, weights);
}
...
void dense_large_nobias(
    ...
    dense_large_rf_gt_nin_rem0_nobias<data_T, res_T, CONFIG_T>(data, res, weights);
}

template<class data_T, class res_T, typename CONFIG_T>
void dense_large_nobias_str(
    data_T data[CONFIG_T::n_in],
    res_T  res[CONFIG_T::n_out],
    hls::stream<typename CONFIG_T::weight_t> weights[CONFIG_T::n_out]) {
    #pragma HLS resource core=AXI4Stream variable=weights
    ...
    dense_large_rf_gt_nin_rem0_nobias_str<data_T, res_T, CONFIG_T>(data, res, weights);
}
...
#pragma HLS UNROLL
    ...
    ReuseLoop:
      for (int ir = 0; ir < rufactor; ir++){
#pragma HLS PIPELINE II=1 rewind
      typename CONFIG_T::accum_t tmpmult;
//#pragma HLS ARRAY_PARTITION variable=tmpmult complete
      //#pragma HLS DEPENDENCE variable=tmpmult inter false
      ...
      int  in_index  = w_index % nin;
      if (w_index >= CONFIG_T::n_in*CONFIG_T::n_out) continue; // check out of bounds
      tmpmult = data[in_index] * weights[w_index];
...
#pragma HLS UNROLL
      ...
      int  out_index = w_index / multfactor;
      if (out_index >= multiplier_limit) continue; // check out of bounds
      mult[out_index] += tmpmult;
    AccumLoop2:
      for (int im = 0; im < multiplier_limit; im++){
#pragma HLS UNROLL
      //int w_index   = ir + rufactor * im;
      //if (w_index >= CONFIG_T::n_in*CONFIG_T::n_out) std::cout << " ---> " << CONFIG_T::n_in*CONFIG_T::n_out << " -- " << im << " -- " << w_index << " -- " << block_factor << std::endl;
      int out_index_m = im/multscale;//w_index  % CONFIG_T::n_out;//w_index % CONFIG_T::n_out;//im/multscale;
      acc[out_index_m] += mult[im];
    }
    }
    Result:
      for(int ires = 0; ires < CONFIG_T::n_out; ires++){
