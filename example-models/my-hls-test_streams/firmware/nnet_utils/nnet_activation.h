...
//       RELU Activation
// *************************************************
//void cast(typename CONFIG_T::accum_t x){
...
{
    const unsigned rufactor = CONFIG_T::reuse_factor;
    const unsigned multiplier_limit = DIV_ROUNDUP(CONFIG_T::n_in,CONFIG_T::reuse_factor);
    data_T datareg; 
    unsigned index=0;
    for (unsigned ii=0; ii<rufactor; ii++) {
      #pragma HLS PIPELINE II=1
      for (unsigned jj=0; jj<multiplier_limit; jj++) {
        #pragma HLS UNROLL
	if(ii*multiplier_limit+jj < CONFIG_T::n_in) { 
	  datareg = data[ii*multiplier_limit+jj];
	  if(datareg < 0) datareg = 0;
	  res[index] = datareg;//cast<data_T,res_T,CONFIG_T>(datareg); 
	  index++;
        }
      }
    }
  ...
  void  relu_stream(hls::stream<data_T> data[CONFIG_T::n_in], hls::stream<res_T> res[CONFIG_T::n_in])
{
    ...
    const int multiplier_limit = DIV_ROUNDUP(CONFIG_T::n_in,CONFIG_T::reuse_factor);
    data_T datareg;
    for (int ii=0; ii<rufactor; ii++) {
      #pragma HLS PIPELINE II=1 rewind
      for (int jj=0; jj<multiplier_limit; jj++) {
        #pragma HLS UNROLL
       	if(ii*multiplier_limit+jj < CONFIG_T::n_in) datareg = data[ii*multiplier_limit+jj].read();
        if(datareg < 0) datareg = 0;
	if(ii*multiplier_limit+jj < CONFIG_T::n_in) res[ii*multiplier_limit+jj].write(datareg);
      }
    }
}
