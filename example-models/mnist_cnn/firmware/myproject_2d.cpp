...
void myproject_2d(
    ...
    for(unsigned i0 = 0; i0 < N_INPUT_1_1; i0++) { 
      ...
      nnet::reset_down_2d<input_t,layer2_t,config2>(layer2_in_row,layer2_in);
      nnet::reset_down_2d<input_t,layer4_t,config4>(layer4_in_row,layer4_in);
      nnet::reset_down_2d<input_t,layer6_t,config6>(layer6_in_row,layer6_in);
      //Loop over Horizontally
      for(unsigned i1 = 0; i1 < N_INPUT_2_1; i1++) { 
       //move the input buffer right one pixel (Perhaps this should go later)
       nnet::shift_right_2d<input_t,input_t,config2>(i1,layer2_in_row,layer2_in);
       ...
       //move the input buffer right one pixel (Perhaps this should go later)
       nnet::shift_right_2d<layer4_t,layer4_t,config4>(i1,layer4_in_row,layer4_in);
