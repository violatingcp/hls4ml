...
void conv_2d_large_cl(
    ...
    //if(CONFIG_T::in_width < 14) std::cout << "---> " << pX << " -- " << pY << " --- " << CONFIG_T::in_width << std::endl;
    //Processs image
    unsigned pLoop = 1;
	...
	pY = pY+1;
	pPass = false;
	for(int i1 = 0; i0 < CONFIG_T::pad_left; i1++) nnet::cnnshiftzero<data_T,res_T,CONFIG_T>(layer_in_row,layer_in);
      }
    }
}
